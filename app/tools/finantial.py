...
class AddTransactionArgs(BaseModel):
    ...
    source_text: Optional[str] = Field(
        default=None, description="Texto original do usuário."
    )
    ...
    category_name: Optional[str] = Field(
        default=None,
        description="Nome da categoria: comida, transporte, moradia, saúde, lazer, contas, besteira, estudo, férias, investimento, presente, outros.",
    )
...
class QueryTransactionsArgs(BaseModel):
    source_text: Optional[str] = Field(
        default=None,
        description="Texto livre para buscar em source_text ou description.",
    )
    date_from_local: Optional[str] = Field(
        default=None, description="Data inicial (YYYY-MM-DD) no fuso de São Paulo."
    )
    date_to_local: Optional[str] = Field(
        default=None, description="Data final (YYYY-MM-DD) no fuso de São Paulo."
    )
    type_name: Optional[str] = Field(
        default=None, description="Tipo: INCOME | EXPENSES | TRANSFER."
    )
    category_name: Optional[str] = Field(
        default=None, description="Nome da categoria (ex: COMIDA, TRANSPORTE)."
    )
...
def update_transaction(
    ...
    if not any(
        ...
        return {
            "status": "error",
            "message": "Nada para atualizar: forneça pelo menos um campo (amount, type, category, description, payment_method, occurred_at).",
        }
    ...
    try:
        ...
        if target_id is None:
            ...
            if not row:
                return {
                    "status": "error",
                    "message": "Nenhuma transação encontrada para os filtros fornecidos.",
                }
            target_id = row[0]
